...
def _piece_data_uri(piece: chess.Piece) -> str:
    """python-chess SVG 기물을 CSS background-image용 data URI로 변환합니다."""
    svg = chess.svg.piece(piece, size=128)
...
def render_clickable_board(
    ...
    """모바일에서도 8×8 비율을 유지하는 클릭형 체스판을 렌더링합니다."""
    selected = st.session_state.free_selected_square
    ...
    files = list(range(8)) if orientation == chess.WHITE else list(reversed(range(8)))
    ranks = list(reversed(range(8))) if orientation == chess.WHITE else list(range(8))

    # Streamlit의 모바일 column stacking을 강제로 해제하고 각 칸을 정사각형으로 고정합니다.
    css_rules = [
        """
        <style>
        .st-key-live_chessboard {
            width: min(100%, 640px);
            max-width: 640px;
            margin-left: auto;
            margin-right: auto;
        ...
        .st-key-live_chessboard div[data-testid="stHorizontalBlock"] {
        ...
        .st-key-live_chessboard div[data-testid="column"] {
        ...
        .st-key-live_chessboard div[data-testid="stButton"] {
            width: 100% !important;
            aspect-ratio: 1 / 1 !important;
            margin: 0 !important;
            padding: 0 !important;
        }
        .st-key-live_chessboard div[data-testid="stButton"] > button {
            ...
            transition: filter .08s ease, box-shadow .08s ease !important;
        }
        .st-key-live_chessboard div[data-testid="stButton"] > button:hover {
            filter: brightness(1.06) !important;
        }
        .st-key-live_chessboard div[data-testid="stButton"] > button:focus-visible {
    ...
    for rank in range(8):
        for file_index in range(8):
            ...
            is_dark = (file_index + rank) % 2 == 0  # a1 is dark
            square_color = "#769656" if is_dark else "#eeeed2"
            hover_color = "#82a35f" if is_dark else "#f5f5df"
            piece_image = f'url("{_piece_data_uri(piece)}")' if piece else "none"

            rule = [
                f'.st-key-free_sq_{square_name} button {{',
                f'background-color: {square_color} !important;',
                f'background-image: {piece_image} !important;',
                '}',
                f'.st-key-free_sq_{square_name} button:hover {{',
                f'background-color: {hover_color} !important;',
                '}',
            ]
            ...
            if last_move is not None and square in (last_move.from_square, last_move.to_square):
                rule.extend([
                    f'.st-key-free_sq_{square_name} button {{',
                    'box-shadow: inset 0 0 0 999px rgba(246, 246, 105, .34) !important;',
                    '}',
                ])
            ...
            if square == selected:
                rule.extend([
                    f'.st-key-free_sq_{square_name} button {{',
                    'box-shadow: inset 0 0 0 5px rgba(255, 215, 0, .95) !important;',
                    'z-index: 3 !important;',
                    '}',
                ])
            ...
            if square in legal_targets:
                if piece is None:
                    rule.extend([
                        f'.st-key-free_sq_{square_name} button::after {{',
                        'content: "";',
                        'position: absolute;',
                        'left: 50%; top: 50%;',
                        'width: 28%; height: 28%;',
                    ...
                    rule.extend([
                        ...
                        'inset: 5%;',
                        'border-radius: 999px;',
                        'border: clamp(3px, .8vw, 7px) solid rgba(20, 20, 20, .32);',
                        'pointer-events: none;',
                        '}',
                    ])

            css_rules.append("".join(rule))

    # 현재 화면 방향에 맞춘 좌표 라벨.
    bottom_rank = ranks[-1]
    left_file = files[0]
    for file_index in files:
        square_name = chess.square_name(chess.square(file_index, bottom_rank))
        label_color = "#eeeed2" if (file_index + bottom_rank) % 2 == 0 else "#769656"
        css_rules.append(
            f'.st-key-free_sq_{square_name} button p::after {{'
            f'content: "{chess.FILE_NAMES[file_index]}";'
            'position:absolute; right:4px; bottom:2px;'
            f'color:{label_color}; font-size:clamp(9px,2.5vw,13px);'
            'font-weight:700; line-height:1; z-index:5;'
            '}'
        )
    for rank in ranks:
        square_name = chess.square_name(chess.square(left_file, rank))
        label_color = "#eeeed2" if (left_file + rank) % 2 == 0 else "#769656"
        css_rules.append(
            f'.st-key-free_sq_{square_name} button p::before {{'
            f'content: "{rank + 1}";'
    ...
    with st.container(key="live_chessboard"):
        for rank in ranks:
            cols = st.columns(8, gap="small")
            for file_index, col in zip(files, cols):
                square = chess.square(file_index, rank)
                square_name = chess.square_name(square)
                piece = board.piece_at(square)
                accessible_label = PIECE_SYMBOLS.get((piece.color, piece.piece_type), "") if piece else "·"

                with col:
                    if st.button(
                        accessible_label,
                        key=f"free_sq_{square_name}",
                        use_container_width=True,
                        help=square_name,
                    ):
                        make_free_move(board, square, promotion_piece)
                        st.rerun()
...
with play_tab:
    ...
    with control3:
        if st.button("↩ 한 수 취소", use_container_width=True, disabled=not st.session_state.free_moves):
            st.session_state.free_moves.pop()
            st.session_state.free_selected_square = None
            st.session_state.free_pending_review = None
            st.session_state.free_last_review = None
