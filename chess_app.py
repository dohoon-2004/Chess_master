...
def render_clickable_board(
    ...
    for rank in ranks:
        ...
        for file_index, col in zip(files, cols):
            ...
            symbol = PIECE_SYMBOLS.get((piece.color, piece.piece_type), "") if piece else ""
            ...
            with col:
                if st.button(
                    label,
                    key=f"free_sq_{square}_{len(st.session_state.free_moves)}",
    ...
    file_names = [chess.FILE_NAMES[f] for f in files]
