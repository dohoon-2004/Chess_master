...
<head>
  ...
  <style>
    :root {
      ...
      --light-hover: #f3f3dc;
      --dark-hover: #82a05f;
      --last: rgba(246, 246, 90, 0.58);
      --selected: rgba(255, 207, 45, 0.98);
...
PIECE_IMAGE_CACHE: dict[tuple[bool, int], str] = {}


def piece_image_uri(piece: chess.Piece) -> str:
    """python-chess SVG 기물을 브라우저에서 바로 표시할 data URI로 변환합니다."""
    key = (piece.color, piece.piece_type)
    cached = PIECE_IMAGE_CACHE.get(key)
    if cached is not None:
        return cached

    svg = chess.svg.piece(piece, size=100)
    encoded = base64.b64encode(svg.encode("utf-8")).decode("ascii")
    uri = f"data:image/svg+xml;base64,{encoded}"
    PIECE_IMAGE_CACHE[key] = uri
    return uri
...
def board_payload(board: chess.Board) -> dict[str, str]:
    """클릭형 보드 컴포넌트에 넘길 칸별 SVG 기물 이미지를 만듭니다."""
    pieces: dict[str, str] = {}
    for square, piece in board.piece_map().items():
        pieces[chess.square_name(square)] = piece_image_uri(piece)
    return pieces
